...
class Servicio(models.Model):
    ...
    def total_abonado(self):
        return self.abonos.aggregate(total=models.Sum("monto"))["total"] or Decimal(
            "0.00"
        )
...
class Abono(models.Model):
    ...
    servicio = models.ForeignKey(
        Servicio, on_delete=models.CASCADE, related_name="abonos"
    )
...
class ReparacionPC(models.Model):
    ...
    servicio = models.OneToOneField(
        Servicio, on_delete=models.CASCADE, related_name="pc"
    )
...
class ReparacionImpresora(models.Model):
    ...
    servicio = models.OneToOneField(
        Servicio, on_delete=models.CASCADE, related_name="impresora"
    )
